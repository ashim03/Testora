...
export async function scheduleAssignment(id: string, data: { publishAt?: string | null; dueAt?: string | null; reminderHoursBefore?: number[] }, actor: { id: string; role: string }): Promise<unknown> {
  ...
  const reminders = [...new Set((data.reminderHoursBefore ?? DEFAULT_REMINDERS).map(Number))].filter((hours) => Number.isFinite(hours) && hours >= 0 && hours <= 168).sort((a, b) => b - a);
  ...
  await audit("SCHEDULE_ASSIGNMENT", { actorId: actor.id, actorRole: actor.role, entityType: "Assignment", entityId: String(assignment._id), after: { publishAt: assignment.publishAt, dueAt: assignment.dueAt, reminderHoursBefore: reminders, status: assignment.status } });
  return assignment;
}
...
export async function processAssignmentSchedules(now = new Date()): Promise<void> {
  const dueForPublishing = await Assignment.find({ deletedAt: null, status: "SCHEDULED", published: false, publishAt: { $ne: null, $lte: now } });
  for (const assignment of dueForPublishing) {
    assignment.published = true;
    assignment.status = "OPEN";
    await assignment.save();
    for (const studentId of assignment.studentIds) await notify(studentId, "ASSIGNMENT_CREATED", "Assignment is now available", `"${assignment.title}" is now open for submission.`, { assignmentId: String(assignment._id), dueAt: assignment.dueAt });
  ...
  const candidates = await Assignment.find({ deletedAt: null, published: true, status: "OPEN", dueAt: { $ne: null, $gt: now }, reminderHoursBefore: { $exists: true, $ne: [] } });
  for (const assignment of candidates) {
    ...
    for (const hours of assignment.reminderHoursBefore || []) {
      ...
      if (now.getTime() < expectedAt || reminderAlreadySent(assignment.reminderSentAt || [], expectedAt)) continue;
      for (const studentId of assignment.studentIds) await notify(studentId, "ASSIGNMENT_DUE_REMINDER", "Assignment deadline reminder", `"${assignment.title}" is due in ${hours} hour${hours === 1 ? "" : "s"}.`, { assignmentId: String(assignment._id), dueAt: assignment.dueAt, hoursBefore: hours });
