...
interface WeakArea { category: string; averageScore: number; difficulty: Difficulty; }

/** Select questions from a student's weakest recent areas without exposing answer keys. */
export async function getAdaptivePractice(
  ...
  const recentResults = await Result.find({ studentId, published: true })
    .sort({ createdAt: -1 }).limit(20).select("category finalScore percentage").lean();
  ...
  const questionGroups = await Promise.all(targetAreas.map((area) => {
  ...
  return { questions: questionGroups.flat().slice(0, limit), weakAreas };
}
